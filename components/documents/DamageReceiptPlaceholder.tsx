...
export interface DamagedLostReceiptItem {
  name: string
  code?: string
  type: 'DAMAGED' | 'LOST'
  quantity: number
...
export interface DamageReceiptPlaceholderProps {
  businessName?: string
  ...
  items?: DamagedLostReceiptItem[]
  totalDamageFee?: number
  totalLossFee?: number
  grandTotalDamage?: number
...
export function DamageReceiptPlaceholder({
  ...
  promptPayValue,
  qrCodeUrl,
  note = 'รับชำระค่าชำรุดและค่าสูญหายของอุปกรณ์เช่า',
}: DamageReceiptPlaceholderProps) {
  ...
  const effectiveTotalDamage = totalDamageFee || items.filter(i => i.type === 'DAMAGED').reduce((sum, i) => sum + i.totalFee, 0)
  const effectiveTotalLoss = totalLossFee || items.filter(i => i.type === 'LOST').reduce((sum, i) => sum + i.totalFee, 0)
  const effectiveGrandTotal = grandTotalDamage || (effectiveTotalDamage + effectiveTotalLoss)
  const effectiveCollected = collectedAmount !== undefined ? collectedAmount : effectiveGrandTotal
  ...
  return (
    <div className="box-border flex h-full w-full flex-col justify-between overflow-hidden bg-white px-[10mm] py-[8mm] font-sans text-[9.5px] leading-[1.3] text-slate-900">
      ...
      <div className="mb-2 flex items-start justify-between gap-3 border-b-2 border-red-700 pb-2.5">
        ...
        <div className="text-right shrink-0">
          <div className="mb-1 inline-block whitespace-nowrap rounded bg-red-700 px-3 py-1 font-black text-[10.5px] uppercase tracking-wider text-white shadow-xs">
            ใบเสร็จรับเงินค่าชำรุด/สูญหาย
          </div>
          <p className="whitespace-nowrap text-[10px] font-bold text-slate-800">
            เลขที่เอกสาร: <span className="text-red-700 font-extrabold">{receiptNo}</span>
          </p>
      ...
      {/* 3. Damaged & Lost Items Table (Exactly 20 Rows) */}
        ...
        <table className="w-full table-fixed border-collapse text-[8.5px] leading-tight border border-slate-400">
          <thead>
            <tr className="bg-slate-900 text-white font-extrabold border-b-2 border-red-700">
              ...
              <th className="w-[14%] border border-slate-700 px-1.5 py-1 text-center">ประเภท</th>
              <th className="w-[10%] border border-slate-700 px-1 py-1 text-center">จำนวน</th>
              <th className="w-[14%] border border-slate-700 px-1.5 py-1 text-right">ค่าปรับ/หน่วย</th>
              <th className="w-[14%] border border-slate-700 px-1.5 py-1 text-right">จำนวนเงิน (บาท)</th>
            </tr>
          </thead>
          <tbody>
            {paddedItems.map((item, index) => {
              ...
              if (item) {
                return (
                  <tr key={index} className="h-[18px] border-b border-slate-300">
                    ...
                    <td className="border-r border-slate-300 px-1.5 py-0.5 text-center">
                      <span
                      ...
                      >
                        {item.type === 'DAMAGED' ? 'ชำรุด (ซ่อม)' : 'สูญหาย (ตัดสต็อก)'}
                      </span>
                    </td>
                    <td className="border-r border-slate-300 px-1 py-0.5 text-center font-mono font-bold">
                      {item.quantity} {item.unit || 'ชิ้น'}
                    </td>
                    <td className="border-r border-slate-300 px-1.5 py-0.5 text-right font-mono tabular-nums">
                      ฿{fmt(item.feePerUnit)}
                    </td>
              ...
              return (
                <tr key={index} className="h-[18px] border-b border-slate-200 bg-white">
      ...
      <div className="mb-2 grid grid-cols-12 gap-3 items-start">
        ...
        <div className="col-span-7 space-y-1.5">
          <div className="rounded border border-slate-300 bg-slate-50/70 p-2 text-[8px] leading-relaxed text-slate-700">
            <p className="font-extrabold text-red-800 mb-0.5 text-[8.5px]">บันทึกและเงื่อนไขการชดใช้ค่าเสียหาย:</p>
            <p>1. รายการชำรุดส่งเข้าแผนกซ่อมบำรุง / รายการสูญหายตัดจำหน่ายออกจากบัญชีทรัพย์สิน</p>
            <p>2. ผู้เช่าชำระค่าชดใช้ครบถ้วนตามรายการข้างต้น ถือว่าสิ้นสุดภาระผูกพันในส่วนความเสียหายนี้</p>
            {note && <p className="mt-0.5 font-bold text-slate-800">หมายเหตุ: {note}</p>}
          </div>
        ...
        <div className="col-span-5 rounded border border-slate-300 bg-slate-50 p-2 text-[8.5px] space-y-0.5">
          <div className="flex justify-between border-b border-slate-200 py-0.5">
            <span className="text-slate-600">รวมค่าซ่อมแซมชำรุด:</span>
            <span className="font-mono font-semibold">฿{fmt(effectiveTotalDamage)}</span>
          </div>
          <div className="flex justify-between border-b border-slate-200 py-0.5">
            <span className="text-slate-600">รวมค่าสินค้าสูญหาย:</span>
            <span className="font-mono font-semibold">฿{fmt(effectiveTotalLoss)}</span>
          </div>
          <div className="flex justify-between border-b border-slate-200 py-0.5 font-bold">
            <span className="text-slate-900">รวมค่าชำรุด/สูญหาย:</span>
            <span className="font-mono text-slate-900">฿{fmt(effectiveGrandTotal)}</span>
          </div>
          <div className="flex justify-between rounded bg-red-700 p-1.5 text-white font-extrabold text-[9.5px] mt-1 shadow-xs">
            ...
            <span className="font-mono">฿{fmt(effectiveCollected)}</span>
          </div>
          ...
          {outstandingRemaining > 0 && (
            <div className="flex justify-between border-t border-slate-200 py-0.5 text-red-700 font-black">
              ...
              <span className="font-mono">฿{fmt(outstandingRemaining)}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="mt-auto grid grid-cols-2 gap-8 border-t border-slate-300 pt-2 text-center text-[8.5px]">
        <div>
          ...
          <p className="text-slate-500 text-[8px]">ผู้ชำระค่าเสียหาย / ผู้เช่า</p>
          <p className="text-slate-400 text-[7.5px]">วันที่ _____/_____/_________</p>
        </div>
        <div>
          <div className="mx-auto mb-1 h-6 w-40 border-b border-dashed border-slate-400" />
          <p className="font-bold text-slate-900">ลงชื่อ (เจ้าหน้าที่ผู้รับเงิน)</p>
